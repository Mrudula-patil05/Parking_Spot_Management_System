    ...
    for (ParkingSpot spot : carPark.getParkingSpots()) {
        ...
        if (spot.isOccupied()) {
            ...
            spotInfo.append("Occupied by ").append(car.getRegistrationNumber())
                    ...
                    .append(", Parking duration: ").append(formatDuration(System.currentTimeMillis() - car.getParkingTime()));
        } else {
...
private void findCarByRegistration() {
    ...
    if (spot != null) {
        ...
        long duration = currentTime - car.getParkingTime();
        JOptionPane.showMessageDialog(this, "Car found in spot " + spot.getSpotID()
                ...
                + "\nParking duration: " + formatDuration(duration));
    } else {
...
private void updatePanelContent(JPanel panel, ParkingSpot spot) {
    ...
    if (spot.isOccupied()) {
        ...
        final JLabel parkingTimeLabel = addOrUpdateLabel(panel, "Parking time: " + formatParkingTime(car.getParkingTime()), 5);
        final JLabel parkingDurationLabel = addOrUpdateLabel(panel, "Parking duration: " + calculateParkingDuration(car.getParkingTime()), 6);

        // Start a separate thread to update the parking duration every second
            ...
            while (spot.isOccupied()) {
                try {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        long currentTime = System.currentTimeMillis();
                        long duration = currentTime - car.getParkingTime();
                        parkingDurationLabel.setText("Parking duration: " + formatDuration(duration));
                    });
        ...
        updater.setDaemon(true); // Allow the thread to be terminated when the application exits
        updater.start();
    } else {
        panel.setBackground(new Color(76, 175, 80)); // Green for unoccupied
    }
...
private JLabel addOrUpdateLabel(JPanel panel, String text, int index) {
